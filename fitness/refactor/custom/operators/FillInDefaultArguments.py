...
class DefaultArgumentsFiller(astor.TreeWalk):
    ...
    def pre_Call(self):
        ...
        if id(self.cur_node) == self.target \
            ...
            print([ast.dump(x) for x in left_over_args])
            print([ast.dump(x) for x in left_over_defaults])
            ...
            u.dump_node(node, "call")
            print()
            u.dump_node(fun_def, "def")
